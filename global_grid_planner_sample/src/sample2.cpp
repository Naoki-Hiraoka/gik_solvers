...
namespace global_grid_planner_sample{
  void sample2(){
    ...
    cnoid::BodyPtr bigBox = new cnoid::Body();
    {
      ...
      shape->setMesh(meshGenerator.generateBox(cnoid::Vector3(0.1,0.1,4.0)));
    ...
    std::vector<std::shared_ptr<ik_constraint2::IKConstraint> > goals;
    {
      ...
      goal->B_link() = nullptr;
      goal->B_localpos().translation() = cnoid::Vector3(-0.5,0.0,0.0);
    ...
    param.forwardScale=0.1;
    param.gridType=global_grid_planner::GGPParam::GridType::XYZTHETA;
    param.gsParam.debugLevel=1;
    param.gsParam.solverType = graph_search2::Param::SolverType::A_STAR;
